...
namespace MsgReader
{
    ...
    public class Reader : IReader
    {
        ...
        private List<string> WriteMsgEmail(Storage.Message message, string outputFolder, bool hyperlinks)
        {
            ...
            // From
            WriteHeaderLineNoEncoding(emailHeader, htmlBody, maxLength, LanguageConsts.EmailFromLabel, message.GetEmailSender(htmlBody, hyperlinks));
            ...
            {
                WriteHeaderLine(emailHeader, htmlBody, maxLength, LanguageConsts.EmailFollowUpLabel, message.Flag.Request);

                // When complete
                if (message.Task.Complete != null && (bool) message.Task.Complete)
                {
                    WriteHeaderLine(emailHeader, htmlBody, maxLength, LanguageConsts.EmailFollowUpStatusLabel, LanguageConsts.EmailFollowUpCompletedText);
            ...
            if (categories != null)
            {
                WriteHeaderLine(emailHeader, htmlBody, maxLength, LanguageConsts.EmailCategoriesLabel, String.Join("; ", categories));
        ...
        private List<string> WriteMsgAppointment(Storage.Message message, string outputFolder, bool hyperlinks)
        {
            ...
            // Subject
            WriteHeaderLine(appointmentHeader, htmlBody, maxLength, LanguageConsts.AppointmentSubjectLabel, message.Subject);

            // Location
            WriteHeaderLine(appointmentHeader, htmlBody, maxLength, LanguageConsts.AppointmentLocationLabel, message.Appointment.Location);
            ...
            {
                WriteHeaderLine(appointmentHeader, htmlBody, maxLength, LanguageConsts.AppointmentRecurrenceTypeLabel, message.Appointment.RecurrenceTypeText);
            }

            // Recurrence patern
            if (!string.IsNullOrEmpty(message.Appointment.RecurrencePatern))
            {
                WriteHeaderLine(appointmentHeader, htmlBody, maxLength, LanguageConsts.AppointmentRecurrencePaternLabel, message.Appointment.RecurrencePatern);
            ...
            {
                WriteHeaderLine(appointmentHeader, htmlBody, maxLength, LanguageConsts.AppointmentClientIntentLabel, message.Appointment.ClientIntentText);
            }
            ...
            if (categories != null)
            {
                WriteHeaderLine(appointmentHeader, htmlBody, maxLength, LanguageConsts.EmailCategoriesLabel, String.Join("; ", categories));
        ...
        private List<string> WriteMsgTask(Storage.Message message, string outputFolder, bool hyperlinks)
        {
            ...
            {
                WriteHeaderLine(taskHeader, htmlBody, maxLength, LanguageConsts.TaskEstimatedEffortLabel, message.Task.EstimatedEffortText);

                // Actual effort
                WriteHeaderLine(taskHeader, htmlBody, maxLength, LanguageConsts.TaskActualEffortLabel, message.Task.ActualEffortText);
            ...
            {
                WriteHeaderLine(taskHeader, htmlBody, maxLength, LanguageConsts.TaskBillingInformationLabel, message.Task.BillingInformation);
            }
        ...
        private List<string> WriteMsgContact(Storage.Message message, string outputFolder, bool hyperlinks)
        {
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.DisplayNameLabel, message.Contact.DisplayName);
            }
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.DepartmentLabel, message.Contact.Department);
            }
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.WorkAddressLabel, message.Contact.WorkAddress);
            }

            // Home address
            if (!string.IsNullOrEmpty(message.Contact.HomeAddress))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.HomeAddressLabel, message.Contact.HomeAddress);
            }

            // Other address
            if (!string.IsNullOrEmpty(message.Contact.OtherAddress))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.OtherAddressLabel, message.Contact.OtherAddress);
            }
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.BusinessTelephoneNumberLabel, message.Contact.BusinessTelephoneNumber);
            }

            // Business telephone number 2
            if (!string.IsNullOrEmpty(message.Contact.BusinessTelephoneNumber2))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.BusinessTelephoneNumber2Label, message.Contact.BusinessTelephoneNumber2);
            }
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.CompanyMainTelephoneNumberLabel, message.Contact.CompanyMainTelephoneNumber);
            }

            // Home telephone number
            if (!string.IsNullOrEmpty(message.Contact.HomeTelephoneNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.HomeTelephoneNumberLabel, message.Contact.HomeTelephoneNumber);
            }
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.CellularTelephoneNumberLabel, message.Contact.CellularTelephoneNumber);
            }

            // Car phone
            if (!string.IsNullOrEmpty(message.Contact.CarTelephoneNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.CarTelephoneNumberLabel, message.Contact.CarTelephoneNumber);
            }

            // Radio
            if (!string.IsNullOrEmpty(message.Contact.RadioTelephoneNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.RadioTelephoneNumberLabel, message.Contact.RadioTelephoneNumber);
            }

            // Beeper
            if (!string.IsNullOrEmpty(message.Contact.BeeperTelephoneNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.BeeperTelephoneNumberLabel, message.Contact.BeeperTelephoneNumber);
            }

            // Callback
            if (!string.IsNullOrEmpty(message.Contact.CallbackTelephoneNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.CallbackTelephoneNumberLabel, message.Contact.CallbackTelephoneNumber);
            }

            // Other
            if (!string.IsNullOrEmpty(message.Contact.OtherTelephoneNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.OtherTelephoneNumberLabel, message.Contact.OtherTelephoneNumber);
            }

            // Primary telephone number
            if (!string.IsNullOrEmpty(message.Contact.PrimaryTelephoneNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.PrimaryTelephoneNumberLabel, message.Contact.PrimaryTelephoneNumber);
            }

            // Telex
            if (!string.IsNullOrEmpty(message.Contact.TelexNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.TelexNumberLabel, message.Contact.TelexNumber);
            }

            // TTY/TDD phone
            if (!string.IsNullOrEmpty(message.Contact.TextTelephone))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.TextTelephoneLabel, message.Contact.TextTelephone);
            }

            // ISDN
            if (!string.IsNullOrEmpty(message.Contact.ISDNNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.ISDNNumberLabel, message.Contact.ISDNNumber);
            }
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.PrimaryFaxNumberLabel, message.Contact.OtherTelephoneNumber);
            }

            // Business fax
            if (!string.IsNullOrEmpty(message.Contact.BusinessFaxNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.BusinessFaxNumberLabel, message.Contact.BusinessFaxNumber);
            }

            // Home fax
            if (!string.IsNullOrEmpty(message.Contact.HomeFaxNumber))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.HomeFaxNumberLabel, message.Contact.HomeFaxNumber);
            }
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.Email1EmailAddressLabel, message.Contact.Email1EmailAddress);
            }

            // E-mail display as
            if (!string.IsNullOrEmpty(message.Contact.Email1DisplayName))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.Email1DisplayNameLabel, message.Contact.Email1DisplayName);
            }

            // E-mail 2
            if (!string.IsNullOrEmpty(message.Contact.Email2EmailAddress))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.Email2EmailAddressLabel, message.Contact.Email2EmailAddress);
            }

            // E-mail display as 2
            if (!string.IsNullOrEmpty(message.Contact.Email2DisplayName))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.Email2DisplayNameLabel, message.Contact.Email2DisplayName);
            }

            // E-mail 3
            if (!string.IsNullOrEmpty(message.Contact.Email3EmailAddress))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.Email3EmailAddressLabel, message.Contact.Email3EmailAddress);
            }

            // E-mail display as 3
            if (!string.IsNullOrEmpty(message.Contact.Email3DisplayName))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.Email3DisplayNameLabel, message.Contact.Email3DisplayName);
            }
            ...
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.SpouseNameLabel, message.Contact.SpouseName);
            }

            // Profession
            if (!string.IsNullOrEmpty(message.Contact.Profession))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.ProfessionLabel, message.Contact.Profession);
            }

            // Assistant
            if (!string.IsNullOrEmpty(message.Contact.AssistantName))
            {
                WriteHeaderLine(contactHeader, htmlBody, maxLength, LanguageConsts.AssistantTelephoneNumberLabel, message.Contact.AssistantName);
            }
            ...
            out List<string> files)
        {
            ...
            foreach (var attachment in message.Attachments)
            {
                ...
                else
                {
                    ...
                    if (attachment is Storage.Message)
                    {
